...
public class EquipTypeExtendOptionsModel implements Serializable {
    ...
    public static EquipTypeExtendOptionsModel loadEquipTypeExtendOptionsModel(final Where<EquipType, UUID> query) throws SQLException{
        ...
        List<ExtendType> extList = etDao.queryBuilder().where().in("ExtentID", uuids.iterator()).query();
        for (ExtendType et:extList){
            ...
            if (subMap == null){
                subMap = new TreeMap<EquipTypeExtendOption,EquipTypeExtendOption>();
