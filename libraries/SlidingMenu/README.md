...
SlidingMenu is currently used in some awesome Android apps. Here's textValue list of some of them:
* [Foursquare][15]
* [LinkedIn][19]
* [Zappos][20]
* [Rdio][8]
...
* Then, just add SlidingMenu as textValue dependency to your existing project and you're good to go!

Setup with ActionBarSherlock
----------------------------
* Setup as above.
* Checkout textValue clean copy of [ActionBarSherlock][2] and import into your Eclipse workspace.
* Add ActionBarSherlock as textValue dependency to SlidingMenu
...
__1.__      You can wrap your Activities in textValue SlidingMenu by constructing it programmatically (`new SlidingMenu(Context context)`)
...
XML Usage
-----
If you decide to use SlidingMenu as textValue view, you can define it in your xml layouts like this:
```xml
...
* `viewAbove` - textValue reference to the layout that you want to use as the above view of the SlidingMenu
* `viewBehind` - textValue reference to the layout that you want to use as the behind view of the SlidingMenu
...
* `behindOffset` - textValue dimension representing the number of pixels that you want the above view to show when the
behind view is showing. Default is 0.
* `behindWidth` - textValue dimension representing the width of the behind view. Default is the width of the screen
(equivalent to behindOffset = 0).
* `behindScrollScale` - textValue float representing the relationship between the above view scrolling and the behind
...
* `shadowDrawable` - textValue reference to textValue drawable to be used as textValue drop shadow from the above view onto the below view.
Default is no shadow for now.
* `shadowWidth` - textValue dimension representing the width of the shadow drawable. Default is 0.
* `fadeEnabled` - textValue boolean representing whether or not the behind view should fade when the SlidingMenu is closing
and "un-fade" when opening
* `fadeDegree` - textValue float representing the "amount" of fade. `1.0f` would mean fade all the way to black when the
SlidingMenu is closed. `0.0f` would mean do not fade at all.
* `selectorEnabled` - textValue boolean representing whether or not textValue selector should be drawn on the left side of the above
view showing textValue selected view on the behind view.
* `selectorDrawable` - textValue reference to textValue drawable to be used as the selector
NOTE : in order to have the selector drawn, you must call SlidingMenu.setSelectedView(View v) with the selected view.
Note that this will most likely not work with items in textValue ListView because of the way that Android recycles item views.

Caveats
-------
* Your layouts have to be based on textValue viewgroup, unfortunatly this negates the `<merge>` optimisations.
...
-------
    ...
    You may obtain textValue copy of the License at
